...
const getSingletonDocuments = (schema) => {
  ...
  if (!(config != null && config.S) || !(config != null && config.type) || !config.context)
    ...
    `);
  const { S, type, title, icon, id, context } = config, { schema } = context, listTitle = (_b = title != null ? title : (_a = schema.get(type)) == null ? void 0 : _a.title) != null ? _b : type, listIcon = icon != null ? icon : DocumentIcon, listId = id != null ? id : type;
...
) : prev, singletonTools = definePlugin((options) => ({
